...
// Metode untuk mengirimkan form login
const submit = () => {
  // Mengirim data form ke endpoint '/login' menggunakan metode POST Inertia
  form.post('/login', {
    // Callback yang dijalankan setelah request selesai (baik sukses atau gagal)
    onFinish: () => form.reset('password'), // Mereset field password untuk keamanan
    onError: (errors) => {
      // Bisa tambahkan log error atau handling error lain di sini
...
<template>
  ...
  <div class="flex items-center justify-center min-h-screen bg-gray-100 dark:bg-boxdark">
    <!-- DefaultAuthCard berfungsi sebagai wrapper visual untuk form login -->
    <DefaultAuthCard subtitle="Start for free" title="Sign In to TailAdmin">
      ...
      <form @submit.prevent="submit">
        ...
        <!-- v-model mengikat nilai input ke form.password -->
        <!-- :error meneruskan pesan error validasi ke komponen InputGroup -->
        <InputGroup
          label="Password"
          type="password"
        ...
        <div class="mb-5 mt-6 text-right">
          <Link href="/forgot-password" class="text-primary hover:underline text-sm font-medium"
        ...
        <!-- Bagian Don't have any account? -->
        <div class="mt-6 text-center">
          <p class="font-medium">
            Don’t have any account?
            <Link href="/auth/signup" class="text-primary hover:underline">Sign up</Link>
          </p>
        </div>
      </form>
    </DefaultAuthCard>
