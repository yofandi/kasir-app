...
// === Ambil props Laravel ===
const page = usePage()
const user = ref(page.props.value.user || {})
...
onMounted(() => {
  ...
  form.value.roles = user.value.roles ? user.value.roles.map((r) => r.name) : []
})

// === METHOD SUBMIT ===
function handleSubmit() {
  Inertia.put(route('apps.users.update', user.value.id), form.value, {
    onSuccess: () => {
      Swal.fire({
        title: 'Success!',
...
<template>
  ...
  <BreadcrumbDefault :pageTitle="'Edit User'" />

  <DefaultCard cardTitle="Edit User" class="shadow-md">
