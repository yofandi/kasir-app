...
// method search
const handleSearch = () => {
  Inertia.get(route('apps.users.index'), {
...
const destroy = (id) => {
  Swal.fire({
    ...
    if (result.isConfirmed) {
      // DELETE via Inertia
      Inertia.delete(route('apps.users.destroy', id), {
        onSuccess: () => {
          Swal.fire({
            title: 'Deleted!',
...
<template>
  ...
  >
    <h2 class="text-white font-semibold text-lg mb-4">Users</h2>
      ...
      <table class="min-w-full table-fixed border-collapse">
        <thead>
          <tr class="bg-gray-800 text-gray-400 text-xs uppercase tracking-wider">
            <th class="w-1/4 text-left px-4 py-3">Name</th>
            <th class="w-1/4 text-left px-4 py-3">Email</th>
            <th class="w-1/2 text-left px-4 py-3">Roles</th>
        ...
        <tbody>
          ...
          >
            <td class="text-white text-sm px-4 py-3">{{ user.name }}</td>
            <td class="text-white text-sm px-4 py-3">{{ user.email }}</td>
    ...
    <!-- pagination -->
    <Pagination :links="users" align="end" />
  </div>
</template>
