...
const props = defineProps(['item', 'index']) // 📥 Ambil props dari parent component (item dan index)

// 🔠 Tipe untuk anak menu (submenu) dalam TypeScript
interface SidebarItem {
  label: string
}

// ✅ Fungsi untuk menangani klik pada menu item sidebar
const handleItemClick = () => {
  // Jika item yang diklik sudah aktif → kosongkan, jika belum → set sebagai aktif
  const pageName = sidebarStore.page === props.item.label ? '' : props.item.label
  // Simpan state halaman aktif ke store
  sidebarStore.page = pageName
  // Debug: tampilkan isi store di console (bisa dihapus di produksi)

  // Jika item memiliki anak (submenu), periksa apakah salah satu sedang dipilih
